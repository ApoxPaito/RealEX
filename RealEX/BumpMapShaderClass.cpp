...
bool BumpMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, (WCHAR*)L"../RealEX/bumpmap.vs", (WCHAR*)L"../RealEX/bumpmap.ps");
	if (!result)
	{
		return false;
	}
...
bool BumpMapShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DXMATRIX worldMatrix,
	DXMATRIX viewMatrix, DXMATRIX projectionMatrix, ID3D11ShaderResourceView** textureArray, DXFLOAT3 lightDirection, DXFLOAT4 diffuseColor)
{
	bool result;
...
bool BumpMapShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DXMATRIX worldMatrix,
	DXMATRIX viewMatrix, DXMATRIX projectionMatrix,	ID3D11ShaderResourceView** textureArray,
	DXFLOAT3 lightDirection, DXFLOAT4 diffuseColor)
{
	HRESULT result;
