...
bool DepthShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, (WCHAR*) L"../RealEX/depth.vs", (WCHAR*) L"../RealEX/depth.ps");
	if (!result)
	{
		return false;
	}
...
bool DepthShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DXMATRIX worldMatrix, DXMATRIX viewMatrix,
...
bool DepthShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DXMATRIX worldMatrix, DXMATRIX viewMatrix, DXMATRIX projectionMatrix)
{
	HRESULT result;
