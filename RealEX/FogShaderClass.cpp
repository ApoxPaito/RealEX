...
bool FogShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, (WCHAR*) L"../RealEX/fog.vs", (WCHAR*) L"../RealEX/fog.ps");
	if (!result)
	{
		return false;
	}
...
bool FogShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DXMATRIX worldMatrix, DXMATRIX viewMatrix,
	DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float fogStart, float fogEnd)
{
	bool result;
...
bool FogShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DCompileFromFile((LPCWSTR)vsFilename, NULL, NULL, "FogVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
	...
	// Compile the vertex shader code.
	result = D3DCompileFromFile((LPCWSTR)psFilename, NULL, NULL, "FogPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
		if (errorMessage)
...
bool FogShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DXMATRIX worldMatrix, DXMATRIX viewMatrix,
	DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float fogStart,
	float fogEnd)
{
	HRESULT result;
