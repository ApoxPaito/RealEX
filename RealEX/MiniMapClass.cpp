...
bool MiniMapClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, HWND hwnd, int screenWidth, int screenHeight, DXMATRIX viewMatrix, float terrainWidth,
	float terrainHeight)
{
	bool result;
	...
	m_mapLocationX = 30;
	m_mapLocationY = screenHeight - 180;
	...
	result = m_MiniMapBitmap->Initialize(device, deviceContext, screenWidth, screenHeight, (char*) "../RealEX/terrain/colorm01.tga", 150, 150);
	if (!result)
	{
	...
	result = m_Border->Initialize(device, deviceContext, screenWidth, screenHeight, (char*) "../RealEX/terrain/border01.tga", 154, 154);
	if (!result)
	{
	...
	result = m_Point->Initialize(device, deviceContext, screenWidth, screenHeight, (char*) "../RealEX/terrain/point01.tga", 3, 3);
	if (!result)
	{
...
bool MiniMapClass::Render(ID3D11DeviceContext* deviceContext, DXMATRIX worldMatrix, DXMATRIX orthoMatrix, ShaderManagerClass* shaderManager)
{
	bool result;
	...
	shaderManager->RenderTextureShader(deviceContext, m_Border->GetIndexCount(), worldMatrix, m_viewMatrix, orthoMatrix, m_Border->GetTexture());
	...
	shaderManager->RenderTextureShader(deviceContext, m_MiniMapBitmap->GetIndexCount(), worldMatrix, m_viewMatrix, orthoMatrix, m_MiniMapBitmap->GetTexture());
	...
	shaderManager->RenderTextureShader(deviceContext, m_Point->GetIndexCount(), worldMatrix, m_viewMatrix, orthoMatrix, m_Point->GetTexture());
