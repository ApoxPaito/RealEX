...
bool ProjectionShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, (WCHAR*) L"../RealEX/projection.vs", (WCHAR*) L"../RealEX/projection.ps");
	if (!result)
	{
		return false;
	}
...
bool ProjectionShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DXMATRIX worldMatrix, DXMATRIX viewMatrix,
	DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, DXFLOAT4 ambientColor, DXFLOAT4 diffuseColor,
	DXFLOAT3 lightDirection, DXMATRIX viewMatrix2, DXMATRIX projectionMatrix2,
...
bool ProjectionShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DXMATRIX worldMatrix, DXMATRIX viewMatrix,
	DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, DXFLOAT4 ambientColor,
	DXFLOAT4 diffuseColor, DXFLOAT3 lightPosition, DXMATRIX viewMatrix2, DXMATRIX projectionMatrix2,
	ID3D11ShaderResourceView* projectionTexture)
{
	HRESULT result;
	...
	// Set the position of the light position constant buffer in the pixel shader.
	bufferNumber = 1;

	// Finally set the light position constant buffer in the vertex shader with the updated values.
