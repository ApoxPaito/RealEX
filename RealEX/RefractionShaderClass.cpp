...
bool RefractionShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, (WCHAR*) L"../RealEX/refraction.vs", (WCHAR*) L"../RealEX/refraction.ps");
	if (!result)
	{
		return false;
	}
...
bool RefractionShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DXMATRIX worldMatrix,
	DXMATRIX viewMatrix, DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture,
	DXFLOAT3 lightDirection, DXFLOAT4 ambientColor, DXFLOAT4 diffuseColor,
	DXFLOAT4 clipPlane)
{
	bool result;
...
bool RefractionShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DXMATRIX worldMatrix, DXMATRIX viewMatrix,
