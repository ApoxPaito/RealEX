...
bool SkyPlaneClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* firstTextureFilename, char* secondTextureFilename, char* perturbTextureFilename)
{
	...
	m_firstTranslation = 0.0f;
	m_secondTranslation = 0.0f;
	...
	result = LoadTextures(device, deviceContext, firstTextureFilename, secondTextureFilename, perturbTextureFilename);
	if (!result)
	{
		return false;
	}
...
void SkyPlaneClass::Frame(float frameTime)
{
	...
	m_firstTranslation += 0.000001f * frameTime;
	if (m_firstTranslation > 1.0f)
	{
		m_firstTranslation -= 1.0f;
	}

	// Increment the texture translation value each frame.
	m_secondTranslation += 0.000002f * frameTime;
	if (m_secondTranslation > 1.0f)
	{
		m_secondTranslation -= 1.0f;
	}
...
ID3D11ShaderResourceView* SkyPlaneClass::GetCloudTexture0()
{
	return m_CloudTexture0->GetTexture();
}
...
float SkyPlaneClass::GetFirstTranslation()
{
	return m_firstTranslation;
}

float SkyPlaneClass::GetSecondTranslation()
{
...
bool SkyPlaneClass::InitializeBuffers(ID3D11Device* device, int skyPlaneResolution)
{
	...
	for (j = 0; j < skyPlaneResolution; j++)
	{
		for (i = 0; i < skyPlaneResolution; i++)
		{
			...
			vertices[index].position = DXFLOAT3(m_skyPlane[index1].x, m_skyPlane[index1].y, m_skyPlane[index1].z);
			vertices[index].texture = DXFLOAT2(m_skyPlane[index1].tu, m_skyPlane[index1].tv);
			indices[index] = index;
			index++;

			// Triangle 1 - Upper Right
			vertices[index].position = DXFLOAT3(m_skyPlane[index2].x, m_skyPlane[index2].y, m_skyPlane[index2].z);
			vertices[index].texture = DXFLOAT2(m_skyPlane[index2].tu, m_skyPlane[index2].tv);
			indices[index] = index;
			index++;

			// Triangle 1 - Bottom Left
			vertices[index].position = DXFLOAT3(m_skyPlane[index3].x, m_skyPlane[index3].y, m_skyPlane[index3].z);
			vertices[index].texture = DXFLOAT2(m_skyPlane[index3].tu, m_skyPlane[index3].tv);
			indices[index] = index;
			index++;
			...
			vertices[index].position = DXFLOAT3(m_skyPlane[index4].x, m_skyPlane[index4].y, m_skyPlane[index4].z);
			vertices[index].texture = DXFLOAT2(m_skyPlane[index4].tu, m_skyPlane[index4].tv);
			indices[index] = index;
			index++;
		}
	}
...
bool SkyPlaneClass::LoadTextures(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* firstTextureFilename, char* secondTextureFilename, char* perturbTextureFilename)
{
	bool result;


	// Create the first cloud texture object.
	m_CloudTexture0 = new TextureClass;
	if (!m_CloudTexture0)
	{
		return false;
	}
	...
	result = m_CloudTexture0->Initialize(device, deviceContext, firstTextureFilename);
	if (!result)
	{
		return false;
	}
	...
	result = m_CloudTexture1->Initialize(device, deviceContext, secondTextureFilename);
	if (!result)
	{
		return false;
	}
	...
	// Initialize the second cloud texture object.
	result = m_PerturbTexture->Initialize(device, deviceContext, perturbTextureFilename);
	if (!result)
	{
		return false;
	}
...
void SkyPlaneClass::ReleaseTextures()
{
	...
	if (m_CloudTexture0)
	{
		m_CloudTexture0->Shutdown();
		delete m_CloudTexture0;
		m_CloudTexture0 = 0;
	}
