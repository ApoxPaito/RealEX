...
bool SkyPlaneShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, (WCHAR*) L"../RealEX/skyplane.vs", (WCHAR*) L"../RealEX/skyplane.ps");
	if (!result)
	{
		return false;
	}
...
bool SkyPlaneShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DXMATRIX worldMatrix, DXMATRIX viewMatrix,
	DXMATRIX projectionMatrix, ID3D11ShaderResourceView* firstTexture, ID3D11ShaderResourceView* secondTexture,
	ID3D11ShaderResourceView* perturbTexture, float firstTranslation, float secondTranslation, float scale, float brightness)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, firstTexture, secondTexture, perturbTexture, firstTranslation,
		secondTranslation, scale, brightness);
	if (!result)
	{
		return false;
	}
...
bool SkyPlaneShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DCompileFromFile(vsFilename, NULL, NULL, "SkyPlaneVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS,
	...
	result = D3DCompileFromFile(psFilename, NULL, NULL, "SkyPlanePixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS,
...
bool SkyPlaneShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DXMATRIX worldMatrix, DXMATRIX viewMatrix, DXMATRIX projectionMatrix,
	...
	float firstTranslation, float secondTranslation, float scale, float brightness)
{
	...
	deviceContext->PSSetShaderResources(0, 1, &firstTexture);
	deviceContext->PSSetShaderResources(1, 1, &secondTexture);
