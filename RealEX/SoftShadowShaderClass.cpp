...
bool SoftShadowShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, (WCHAR*) L"../RealEX/softshadow.vs", (WCHAR*) L"../RealEX/softshadow.ps");
	if (!result)
	{
		return false;
	}
...
bool SoftShadowShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DXMATRIX worldMatrix, DXMATRIX viewMatrix,
	DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* shadowTexture,
	DXFLOAT3 lightPosition, DXFLOAT4 ambientColor, DXFLOAT4 diffuseColor)
{
	bool result;
...
bool SoftShadowShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DXMATRIX worldMatrix, DXMATRIX viewMatrix,
	DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* shadowTexture,
	DXFLOAT3 lightPosition, DXFLOAT4 ambientColor, DXFLOAT4 diffuseColor)
{
	HRESULT result;
