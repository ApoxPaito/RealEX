...
bool SpecMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, (WCHAR*)L"../RealEX/specmap.vs", (WCHAR*)L"../RealEX/specmap.ps");
	if (!result)
	{
		return false;
	}
...
bool SpecMapShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DXMATRIX worldMatrix,
	DXMATRIX viewMatrix, DXMATRIX projectionMatrix, ID3D11ShaderResourceView** textureArray, DXFLOAT3 lightDirection,
	DXFLOAT4 diffuseColor, DXFLOAT3 cameraPosition, DXFLOAT4 specularColor,
	float specularPower)
{
	bool result;
...
bool SpecMapShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DCompileFromFile(vsFilename, NULL, NULL, "SpecMapVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS,
	...
	result = D3DCompileFromFile(psFilename, NULL, NULL, "SpecMapPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS,
...
bool SpecMapShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DXMATRIX worldMatrix,
	DXMATRIX viewMatrix, DXMATRIX projectionMatrix,
	ID3D11ShaderResourceView** textureArray, DXFLOAT3 lightDirection,
	DXFLOAT4 diffuseColor, DXFLOAT3 cameraPosition, DXFLOAT4 specularColor,
	float specularPower)
{
	HRESULT result;
