...
bool TerrainShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, (WCHAR*) L"../RealEX/terrain.vs", (WCHAR*) L"../RealEX/terrain.ps");
	if (!result)
	{
		return false;
	}
...
bool TerrainShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DXMATRIX worldMatrix, DXMATRIX viewMatrix,
	DXMATRIX projectionMatrix, DXFLOAT4 diffuseColor, DXFLOAT3 lightDirection,
	ID3D11ShaderResourceView* colorTexture, ID3D11ShaderResourceView* normalMapTexture)
{
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, diffuseColor, lightDirection, colorTexture,
		normalMapTexture);
	if (!result)
	{
		return false;
...
bool TerrainShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DXMATRIX worldMatrix, DXMATRIX viewMatrix,
	DXMATRIX projectionMatrix, DXFLOAT4 diffuseColor, DXFLOAT3 lightDirection, ID3D11ShaderResourceView* colorTexture,
	ID3D11ShaderResourceView* normalMapTexture, float textureBrightness)
{
	bool result;
...
bool TerrainShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DXMATRIX worldMatrix, DXMATRIX viewMatrix,
	DXMATRIX projectionMatrix, DXFLOAT4 ambientColor, DXFLOAT4 diffuseColor, DXFLOAT3 lightDirection,
...
bool TerrainShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DXMATRIX worldMatrix, DXMATRIX viewMatrix,
	DXMATRIX projectionMatrix, DXFLOAT4 ambientColor, DXFLOAT4 diffuseColor, DXFLOAT3 lightDirection,
	ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* detailTexture, ID3D11ShaderResourceView* normalTexture)
{
	bool result;
...
bool TerrainShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DXMATRIX worldMatrix, DXMATRIX viewMatrix, DXMATRIX projectionMatrix, DXFLOAT3 lightDirection,
	...
	ID3D11ShaderResourceView* alphaTexture1, ID3D11ShaderResourceView* normalMap1, ID3D11ShaderResourceView* normalMap2)
{
	...
	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, lightDirection, colorTexture1, colorTexture2, colorTexture3, colorTexture4, alphaTexture1,
...
bool TerrainShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DCompileFromFile((LPCWSTR)vsFilename, NULL, NULL, "TerrainVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
	...
	result = D3DCompileFromFile((LPCWSTR)psFilename, NULL, NULL, "TerrainPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
		if (errorMessage)
...
bool TerrainShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DXMATRIX worldMatrix, DXMATRIX viewMatrix, DXMATRIX projectionMatrix,
	DXFLOAT4 diffuseColor, DXFLOAT3 lightDirection, ID3D11ShaderResourceView* colorTexture, ID3D11ShaderResourceView* normalMapTexture)
{
	HRESULT result;
...
bool TerrainShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DXMATRIX worldMatrix, DXMATRIX viewMatrix, DXMATRIX projectionMatrix,
	DXFLOAT4 diffuseColor, DXFLOAT3 lightDirection, ID3D11ShaderResourceView* colorTexture, ID3D11ShaderResourceView* normalMapTexture, float textureBrightness)
{
	HRESULT result;
	D3D11_MAPPED_SUBRESOURCE mappedResource;
	unsigned int bufferNumber;
	...
	dataPtr2->diffuseColor = diffuseColor;
	dataPtr2->lightDirection = lightDirection;
	dataPtr2->padding = textureBrightness; // Texture brightness
...
bool TerrainShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DXMATRIX worldMatrix, DXMATRIX viewMatrix, DXMATRIX projectionMatrix,
	DXFLOAT4 ambientColor, DXFLOAT4 diffuseColor, DXFLOAT3 lightDirection, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* detailTexture)
{
	HRESULT result;
	D3D11_MAPPED_SUBRESOURCE mappedResource;
	unsigned int bufferNumber;
