...
bool TranslateShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, (WCHAR*) L"../RealEX/translate.vs", (WCHAR*) L"../RealEX/translate.ps");
	if (!result)
	{
		return false;
	}
...
bool TranslateShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DXMATRIX worldMatrix,
...
bool TranslateShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DCompileFromFile((LPCWSTR)vsFilename, NULL, NULL, "TranslateVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
	...
	result = D3DCompileFromFile((LPCWSTR)psFilename, NULL, NULL, "TranslatePixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
		if (errorMessage)
...
bool TranslateShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DXMATRIX worldMatrix, DXMATRIX viewMatrix,
