...
bool WaterClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* textureFilename, float waterHeight, float waterRadius)
{
	bool result;
	...
	m_refractionTint = DXFLOAT4(0.0f, 0.8f, 1.0f, 1.0f);
...
DXFLOAT2 WaterClass::GetNormalMapTiling()
{
...
DXFLOAT4 WaterClass::GetRefractionTint()
{
...
bool WaterClass::InitializeBuffers(ID3D11Device* device, float waterRadius)
{
	...
	vertices[0].position = DXFLOAT3(-waterRadius, 0.0f, waterRadius);  // Top left.
	vertices[0].texture = DXFLOAT2(0.0f, 0.0f);

	vertices[1].position = DXFLOAT3(waterRadius, 0.0f, waterRadius);  // Top right.
	vertices[1].texture = DXFLOAT2(1.0f, 0.0f);

	vertices[2].position = DXFLOAT3(-waterRadius, 0.0f, -waterRadius);  // Bottom left.
	vertices[2].texture = DXFLOAT2(0.0f, 1.0f);

	vertices[3].position = DXFLOAT3(-waterRadius, 0.0f, -waterRadius);  // Bottom left.
	vertices[3].texture = DXFLOAT2(0.0f, 1.0f);

	vertices[4].position = DXFLOAT3(waterRadius, 0.0f, waterRadius);  // Top right.
	vertices[4].texture = DXFLOAT2(1.0f, 0.0f);

	vertices[5].position = DXFLOAT3(waterRadius, 0.0f, -waterRadius);  // Bottom right.
	vertices[5].texture = DXFLOAT2(1.0f, 1.0f);
