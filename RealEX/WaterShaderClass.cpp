...
bool WaterShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, (WCHAR*) L"../RealEX/water.vs", (WCHAR*) L"../RealEX/water.ps");
	if (!result)
	{
		return false;
	}
...
bool WaterShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DXMATRIX worldMatrix, DXMATRIX viewMatrix,
...
bool WaterShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DCompileFromFile((LPCWSTR)vsFilename, NULL, NULL, "WaterVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
	...
	result = D3DCompileFromFile((LPCWSTR)psFilename, NULL, NULL, "WaterPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
		if (errorMessage)
...
bool WaterShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DXMATRIX worldMatrix, DXMATRIX viewMatrix, DXMATRIX projectionMatrix,
	DXMATRIX reflectionMatrix, ID3D11ShaderResourceView* refractionTexture,
	ID3D11ShaderResourceView* reflectionTexture, ID3D11ShaderResourceView* normalTexture,
	DXFLOAT3 cameraPosition, DXFLOAT2 normalMapTiling, float waterTranslation, float reflectRefractScale,
	DXFLOAT4 refractionTint, DXFLOAT3 lightDirection, float specularShininess)
{
	HRESULT result;
	...
	dataPtr2->padding2 = DXFLOAT2(0.0f, 0.0f);
	...
	dataPtr3->padding = DXFLOAT2(0.0f, 0.0f);
